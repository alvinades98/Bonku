...
export default function DashboardSidebar({ mobileOpen, onCloseMobile }: Props) {
  ...
  const sidebarContent = (
    <>
      <div className="flex items-center gap-3 px-6 py-5 border-b border-slate-200/80">
        ...
        <span className="font-bold text-slate-800 text-lg tracking-tight">InvoiceGen</span>
      </div>
      ...
      <div className="px-3 pb-4 border-t border-slate-200/80 pt-4">
        {loading ? (
          <div className="px-3 py-2">
            <div className="h-4 w-24 bg-slate-200 rounded animate-pulse" />
            <div className="h-3 w-32 bg-slate-100 rounded animate-pulse mt-2" />
          </div>
        ) : user ? (
  ...
  return (
    <>
      ...
      <aside
        className={cn(
          'fixed top-0 left-0 h-full w-72 bg-white border-r border-slate-200 z-50 flex flex-col transition-transform duration-300 ease-in-out lg:hidden',
      ...
      {/* Desktop sidebar */}
      <aside className="hidden lg:flex lg:flex-col fixed left-0 top-0 h-full w-64 bg-white border-r border-slate-200 z-40">
        {sidebarContent}
      </aside>
    </>
  )
